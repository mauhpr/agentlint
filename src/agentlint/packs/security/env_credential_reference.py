...
_CRED_FILE_ENV_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (
        re.compile(
            r"(?:PROXY|CREDENTIAL|SECRET|PASSWORD|TOKEN|KEY|CERT|AUTH)\w*FILE\s*=\s*(?!Secret:)(?!secretmanager:)\S+",
            re.I,
        ),
...
class EnvCredentialReference(Rule):
    ...
    def evaluate(self, context: RuleContext) -> list[Violation]:
        ...
        if context.tool_name == "Bash":
            ...
            content = (
                context.tool_input.get("new_string") or context.tool_input.get("content") or ""
            )
