...
_FIREWALL_PATTERNS: list[tuple[re.Pattern[str], str]] = [
    ...
    (
        re.compile(r"\bufw\s+allow\s+from\s+any\b|\bufw\s+allow\b.*0\.0\.0\.0/0", re.I),
        "ufw allow from any",
    ),
    # firewalld
    (
        re.compile(r"\bfirewall-cmd\b.*(?:--add-port|--add-service).*--permanent", re.I),
        "firewalld permanent rule",
    ),
    (
        re.compile(r"\bfirewall-cmd\b.*--zone=public.*--add-(?:port|service)", re.I),
        "firewalld public zone rule",
    ),
    # Route/DNS mutations
