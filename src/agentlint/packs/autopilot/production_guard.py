...
class ProductionGuard(Rule):
    ...
    def evaluate(self, context: RuleContext) -> list[Violation]:
        ...
        allowed_projects: list[str] = [
            p.lower() for p in get_rule_setting(context.config, self.id, "allowed_projects", [])
        ]
        allowed_hosts: list[str] = [
            h.lower() for h in get_rule_setting(context.config, self.id, "allowed_hosts", [])
        ]
        ...
        for pattern, label in _PROD_PATTERNS:
            ...
            if match:
                ...
                has_staging_context = not strict_mode and _STAGING_CONTEXT_RE.search(matched_text)
