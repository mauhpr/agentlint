...
_DOCKER_RISKY_PATTERNS: list[tuple[re.Pattern[str], str, Severity]] = [
    ...
    (
        re.compile(r"\bdocker\s+volume\s+(?:rm|remove)\b", re.I),
    ...
    (
        re.compile(r"\bdocker\s+(?:container\s+)?rm\s+(?:-f|--force)\b", re.I),
    ...
    (
        re.compile(r"\bdocker\s+run\b.*--pid=host\b", re.I),
        "docker run --pid=host (host PID namespace)",
        Severity.ERROR,
    ),
    (
        re.compile(r"\bdocker\s+run\b.*--network=host\b", re.I),
        "docker run --network=host",
        Severity.WARNING,
    ),
    (
        re.compile(r"\bdocker\s+run\b.*-v\s+/var/run/docker\.sock", re.I),
        "docker.sock mount (host root access via socket)",
        Severity.ERROR,
    ),
    (
        re.compile(r"\bdocker\s+run\b.*-v\s+/:/", re.I),
        "root filesystem mount into container",
        Severity.ERROR,
    ),
]
