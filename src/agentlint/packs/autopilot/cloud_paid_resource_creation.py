...
_PAID_RESOURCE_OPS: list[tuple[re.Pattern[str], str, str]] = [
    (
        re.compile(r"\bgcloud\s+compute\s+addresses\s+create\b", re.I),
        "GCP static IP address",
        "~$0.01/hr each",
    ),
    (
        re.compile(r"\bgcloud\s+compute\s+disks\s+create\b", re.I),
        "GCP persistent disk",
        "charged by GB/month",
    ),
    (
        re.compile(r"\bgcloud\s+compute\s+instances\s+create\b", re.I),
        "GCP Compute Engine instance",
        "ongoing VM cost",
    ),
    (
        re.compile(r"\bgcloud\s+sql\s+instances\s+create\b", re.I),
        "GCP Cloud SQL instance",
        "ongoing DB cost",
    ),
    (
        re.compile(r"\bgcloud\s+container\s+clusters\s+create\b", re.I),
        "GCP GKE cluster",
        "ongoing cluster cost",
    ),
    (
        re.compile(r"\baws\s+ec2\s+allocate-address\b", re.I),
        "AWS Elastic IP",
        "~$0.005/hr when idle",
    ),
    (
        re.compile(r"\baws\s+ec2\s+run-instances\b", re.I),
        "AWS EC2 instance",
        "ongoing instance cost",
    ),
    (
        re.compile(r"\baws\s+rds\s+create-db-instance\b", re.I),
        "AWS RDS instance",
        "ongoing DB cost",
    ),
    (
        re.compile(r"\baws\s+eks\s+create-cluster\b", re.I),
        "AWS EKS cluster",
        "ongoing cluster cost",
    ),
